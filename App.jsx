...
const SUPABASE_ANON_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYm16enRyZ3p6dXBrbHdtdHpuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ2MjU4MTEsImV4cCI6MjA5MDIwMTgxMX0.c5WFSAstu-W_UHI8LymjSMORNFR3K2N_cTgkzu3bmf0";
const ADMIN_EMAILS = [
  "scrapbookjoey@gmail.com",
...
const scheduleOverrides = {
  ...
  "2026-03-20": {
    volunteers: ["Alan", "Jay"],
    notes: ["No Karen", "No Ryann", "No Lacy"],
  },
  ...
  "2026-04-17": {
    volunteers: ["Alan", "Lacy"],
    notes: ["Education WIP 4-6?"],
  },
  ...
  "2026-05-02": {
    volunteers: ["Jay"],
    notes: ["No Gail", "Closing early at 1:30 PM"],
    docentTime: "10:00 AM - 1:30 PM",
  },
  "2026-05-08": { volunteers: ["Gail", "Alan"] },
  "2026-05-09": {
    volunteers: ["Jay"],
    labels: ["250 Grand Opening"],
    notes: ["No Ryann", "No Lacy"],
  },
...
const specialEvents = [
  ...
  {
    dateKey: "2026-05-09",
    event: "America 250 Grand Opening",
    time: "11:00 AM - 4:00 PM",
    role: "Special Event Helper 1",
    slotCount: 1,
  },
  {
    dateKey: "2026-05-09",
    event: "America 250 Grand Opening",
    time: "11:00 AM - 4:00 PM",
    role: "Special Event Helper 2",
    slotCount: 1,
  },
  {
    dateKey: "2026-06-19",
    event: "Juneteenth Program",
    time: "11:00 AM - 4:00 PM",
  ...
  {
    dateKey: "2026-07-04",
    event: "Independence Day Visitors",
    time: "11:00 AM - 4:00 PM",
    role: "Guest Support",
    slotCount: 1,
  },
...
function getDefaultDocentTime(date) {
  return date.getDay() === 5 ? "11:00 AM - 4:00 PM" : "10:00 AM - 3:00 PM";
}
...
function generateYearlyDocentSlots(year) {
  ...
  for (let month = 0; month < 12; month += 1) {
    ...
    for (let day = 1; day <= daysInMonth; day += 1) {
      ...
      if (isLukeSaturday(date) && !volunteers.includes("Luke")) {
        volunteers.push("Luke");
      }
      ...
      roleNames.forEach((roleName, index) => {
        ...
        slots.push({
          ...
          dateKey,
          time: override.docentTime || getDefaultDocentTime(date),
          role: roleName,
          spots: 1,
          ...
          description: assignedVolunteer
            ? roleName +
              " is currently assigned to " +
              assignedVolunteer +
              "."
            : "Volunteer coverage for " + roleName + " at the museum.",
        });
      });
    }
  }
...
function buildInitialSlots() {
  const docentSlots = ACTIVE_YEARS.flatMap((year) =>
    generateYearlyDocentSlots(year)
  );
...
function normalizeSlotFromDb(row) {
  return {
    ...
    description:
      row.description ||
      "Volunteer coverage for " + (row.role || "this role") + ".",
  };
}
...
function AdminRowActions({ slot, onChanged }) {
  ...
  return (
    <button style={ui.buttonOutline} onClick={remove}>
      Delete
    </button>
  );
}
...
function AdminAddSlot({ slots, onAdded }) {
  ...
  const [time, setTime] = useState("11:00 AM - 4:00 PM");
  ...
  const loadSlot = (id) => {
    ...
    setTime(slot.time || "11:00 AM - 4:00 PM");
    setRole(slot.role || "");
    setMsg("");
  };
  ...
  const save = async () => {
    ...
    if (mode === "edit" && selectedId) {
      ...
      const ownerEmail = getOwnerEmailFromDescription(
        existing && existing.description ? existing.description : ""
      );

      const description =
        existing && existing.assignedVolunteer
          ? role +
            " is currently assigned to " +
            existing.assignedVolunteer +
            "." +
            (ownerEmail ? " " + OWNER_MARKER + ownerEmail : "")
          : "Volunteer coverage for " + role + " at the museum.";

      const { error } = await supabase
        .from(TABLE_NAME)
        .update({ ...baseRow, description })
    ...
    const row = {
      ...
      signedUp: 0,
      assignedVolunteer: "",
      description: "Volunteer coverage for " + role + " at the museum.",
    };
    ...
    <Card>
      <div style={ui.cardHeader}>
        <h3 style={ui.title}>
          ...
          />{" "}
          Admin: Add, edit, or delete slots
        </h3>
      </div>
      <div style={ui.cardContent}>
        ...
        <div style={{ ...ui.row, marginTop: 12 }}>
          <button style={ui.button} onClick={save}>
            {mode === "edit" ? "Save changes" : "Add slot"}
          </button>
          <button style={ui.buttonOutline} onClick={resetForm}>
            Clear
          </button>
          {mode === "edit" && (
            <button style={ui.buttonOutline} onClick={remove}>
        ...
        <div style={{ ...ui.info, marginTop: 16 }}>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (e.target.value) loadSlot(e.target.value);
            }}
          >
            <option value="">Choose a slot to edit or delete</option>
            {slots.map((slot) => (
              <option key={slot.id} value={slot.id}>
                {slot.date} | {slot.time} | {slot.event} | {slot.role}
              </option>
            ))}
          </select>
        </div>
...
export default function VolunteerSignupApp() {
  ...
  const [syncStatus, setSyncStatus] = useState(
    "Connecting to shared volunteer schedule..."
  );
  ...
  useEffect(() => {
    ...
    const matches = slots.filter((slot) => {
      ...
      return (
        normalizeName(getFirstName(slot.assignedVolunteer)) ===
        normalizedFirstName
      );
    });
    ...
    const load = async () => {
      ...
      if (!data || data.length === 0) {
        ...
        const { error: seedError } = await supabase
          .from(TABLE_NAME)
          .insert(seedSlots.map(serializeSlotForDb));

        if (seedError) {
          setSlots(seedSlots);
          setSyncStatus(
            "Supabase connected, but initial seeding failed: " +
              seedError.message
          );
          setIsLoading(false);
          return;
  ...
  const handleSignup = async () => {
    ...
    const { error } = await supabase
      .from(TABLE_NAME)
      .update({
        signedUp: selectedSlot.signedUp + 1,
  ...
  const filteredSlots = useMemo(() => {
    ...
    const upcomingSlots = slots.filter(
      (slot) => new Date(slot.dateKey + "T00:00:00") >= today
    );
    ...
    return upcomingSlots.filter((slot) => {
      const hay = [
        ...
        slot.time,
        slot.role,
        getVisibleDescription(slot.description),
        slot.assignedVolunteer,
      ]
        .join(" ")
        .toLowerCase();
  ...
  const groupedSlots = useMemo(() => {
    const groups = filteredSlots.reduce((acc, slot) => {
      ...
      if (!acc[key]) {
        acc[key] = {
          ...
          event: slot.event,
          time: slot.time,
          monthLabel: getMonthLabel(slot.dateKey),
          isTeaServiceGroup: isTeaService,
          teaServiceLabel: isTeaService ? baseRole : "",
          slots: [],
        };
      }
    ...
    return selectedMonth === "all"
      ? ordered
      : ordered.filter((group) => group.monthLabel === selectedMonth);
  }, [filteredSlots, selectedMonth]);

  const availableMonths = useMemo(
    () => Array.from(new Set(filteredSlots.map((slot) => getMonthLabel(slot.dateKey)))),
    [filteredSlots]
  );

  useEffect(() => {
    if (selectedMonth !== "all" && availableMonths.indexOf(selectedMonth) === -1) {
    ...
    setExpandedDates((prev) => {
      ...
      groupedSlots.forEach((group, index) => {
        const key =
          ...
          group.time +
          "__" +
          (group.isTeaServiceGroup ? group.teaServiceLabel : "default");
    ...
    return (
      <div style={ui.page}>
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          <Card>
            <div style={ui.cardContent}>
              <div style={ui.badge}>Password Reset</div>
              <h1 style={{ fontSize: 34, marginTop: 16, marginBottom: 12 }}>
                Set a new password
              </h1>
              <form
              ...
              {authMessage ? (
                <div style={{ ...ui.info, marginTop: 16 }}>{authMessage}</div>
              ) : null}
              {authError ? (
                <div style={{ ...ui.info, marginTop: 16 }}>{authError}</div>
              ) : null}
            </div>
          </Card>
        </div>
          ...
          <Card>
            <div style={ui.cardContent}>
              ...
              <p style={{ color: "#57534e", lineHeight: 1.5 }}>
                This schedule is private for approved volunteers. Existing
                volunteers can sign in, and new volunteers can create an account
                with the museum volunteer access code.
              </p>

              <div style={{ ...ui.row, marginTop: 20 }}>
                <button
                  style={authMode === "signin" ? ui.button : ui.buttonOutline}
                  onClick={() => {
                    setAuthMode("signin");
                    setAuthError("");
                    setAuthMessage("");
                ...
                <button
                  style={authMode === "signup" ? ui.button : ui.buttonOutline}
                  onClick={() => {
                    setAuthMode("signup");
                    setAuthError("");
                    setAuthMessage("");
              ...
              {authMode === "signin" ? (
                ...
                >
                  ...
                  <input
                    ...
                    placeholder="Museum volunteer access code"
                  />
                  <button type="submit" style={ui.button}>
                    Create account
                  </button>
    ...
    <div style={ui.page}>
      <div style={ui.shell}>
        {claimableSlots.length > 0 ? (
          <Card>
            <div style={ui.cardHeader}>
              <h3 style={ui.title}>Claim your existing shifts</h3>
            </div>
            <div style={ui.cardContent}>
              <p style={ui.small}>
                We found existing shifts that look like they belong to{" "}
                {(session.user.user_metadata &&
                  session.user.user_metadata.full_name) ||
                  session.user.email}
                . Claim them so only you can manage them.
              </p>
                ...
                {claimableSlots.map((slot) => (
                  <div key={slot.id} style={ui.shiftRow}>
                    <div>
                      ...
                      <div style={ui.small}>
                        {slot.time} • {slot.event} • {slot.role}
                      </div>
                      <div style={ui.small}>
                        Currently listed as {slot.assignedVolunteer}
                      </div>
                    </div>
                    <button
                      style={ui.button}
                      onClick={() => claimShiftOwnership(slot)}
                    >
                      Claim this shift
                    </button>
                  </div>
              ...
              {claimableSlots.length > 1 ? (
                <div style={{ marginTop: 12 }}>
                  <button
                    style={ui.buttonOutline}
                    onClick={claimAllMatchingShifts}
                  >
                    Claim all matching shifts
                  </button>
                </div>
              ) : null}
            </div>
        ...
        >
          <Card>
            <div style={ui.cardContent}>
              ...
              <h1 style={{ fontSize: 40, marginTop: 14, marginBottom: 12 }}>
                Pick a shift without the usual digital obstacle course.
              </h1>
              <p style={{ color: "#57534e", lineHeight: 1.5 }}>
                Volunteers can browse open roles, search by event, and claim a
                spot in a clean, friendly flow.
              </p>
          ...
          <Card>
            <div style={ui.cardHeader}>
              <div style={{ ...ui.row, justifyContent: "space-between" }}>
                ...
                <button style={ui.buttonOutline} onClick={handleSignOut}>
                  Sign out
                </button>
              </div>
            </div>
            <div style={ui.cardContent}>
              <div style={{ position: "relative" }}>
                ...
                />
                <input
                  style={{ ...ui.input, paddingLeft: 36 }}
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                  placeholder="Search by role, event, or date"
                />
              </div>

              <div style={{ marginTop: 16 }}>
                <div
                  style={{ ...ui.small, fontWeight: 700, marginBottom: 8 }}
                >
                  Filter by month
                </div>
                <div style={ui.row}>
                  <button
                    style={
                      selectedMonth === "all" ? ui.button : ui.buttonOutline
                    }
                  ...
                  {availableMonths.map((month) => (
                    <button
                      key={month}
                      style={
                        selectedMonth === month ? ui.button : ui.buttonOutline
                      }
              ...
              <p style={{ ...ui.small, marginTop: 16 }}>
                Showing only current and upcoming docent dates, grouped by day
                with expandable shifts.
              </p>
              <div style={{ ...ui.info, marginTop: 12 }}>
                {isLoading ? "Loading shared schedule..." : syncStatus}
              </div>
            </div>
          </Card>
        </motion.div>
        ...
        {success ? (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <Card>
              <div style={{ ...ui.cardContent, ...ui.row }}>
        ...
        <div style={ui.twoCol}>
          <div style={{ display: "grid", gap: 16 }}>
            {isAdmin(session.user.email) ? (
              <AdminAddSlot slots={slots} onAdded={persistSlots} />
            ) : null}
                      ...
                      <div style={{ display: "grid", gap: 12 }}>
                        <div style={{ ...ui.row, justifyContent: "space-between" }}>
                          <div style={ui.row}>
                            ...
                            {group.isTeaServiceGroup ? (
                              <span style={ui.mutedBadge}>
                                {group.teaServiceLabel} cluster
                              </span>
                            ) : null}
                          </div>
                          <span style={ui.mutedBadge}>
                        ...
                        <div style={ui.row}>
                          ...
                          <span style={ui.mutedBadge}>
                            {openCount} open {openCount === 1 ? "spot" : "spots"}
                          </span>
                        </div>
                      </div>
                    </button>
                        ...
                        <div style={{ display: "grid", gap: 12 }}>
                          {group.slots.map((slot) => {
                            ...
                            const ownedByCurrentUser =
                              getOwnerEmailFromDescription(slot.description) ===
                              session.user.email;
                              ...
                              <div key={slot.id} style={ui.shiftRow}>
                                <div>
                                  <div style={ui.row}>
                                    ...
                                    {ownedByCurrentUser ? (
                                      <span style={ui.mutedBadge}>Your shift</span>
                                    ) : null}
                                  </div>
                                  <div style={{ ...ui.small, marginTop: 4 }}>
                                    {getVisibleDescription(slot.description)}
                                  </div>
                                  <div style={{ ...ui.small, marginTop: 4 }}>
                                    {isFull
                                      ? "Assigned to " + slot.assignedVolunteer
                                ...
                                <div style={ui.row}>
                                  ...
                                  {isFull && ownedByCurrentUser ? (
                                    <button
                                      style={ui.buttonOutline}
                                      onClick={() => {
                                        setSelectedSlot(null);
                                        setSelectedFilledSlot(slot);
                                      }}
                                    >
                                      Un-sign-up
                                    </button>
          ...
          <div
            style={{ position: "sticky", top: 24, display: "grid", gap: 16 }}
            ref={claimCardRef}
          >
            <Card>
              <div style={ui.cardHeader}>
                <h3 style={ui.title}>
                  {selectedSlot
                    ? "Claim this volunteer spot"
                    : selectedFilledSlot
                    ? "Remove me from this shift"
                    : "Choose a slot to begin"}
                </h3>
              </div>
              <div style={ui.cardContent}>
                {selectedSlot ? (
                  ...
                  <div style={{ display: "grid", gap: 12 }}>
                    <div style={ui.info}>
                      ...
                      <div style={{ marginTop: 8 }}>
                        ...
                        {selectedFilledSlot.assignedVolunteer}
                      </div>
                    </div>
                    <button
                      style={ui.buttonOutline}
                      onClick={() => handleUnsignup(selectedFilledSlot.id)}
                    >
                      <XCircle
                        size={16}
                        style={{ marginRight: 8, verticalAlign: "middle" }}
                  ...
                  <div style={ui.info}>
                    Select any open role to sign up. You can only un-sign your
                    own shifts.
                  </div>
                )}
              </div>
